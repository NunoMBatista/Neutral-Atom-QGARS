...
\begin{figure}[!tb]
    ...
    \resizebox{\columnwidth}{!}{%
        \begin{circuitikz}
            ...
            \draw(8,12.25) rectangle  node {\Huge $f_{\bm\theta}(\bm{x})$} (10.5,9.75);
            ...
            \draw(17.75,12.25) rectangle  node {\Huge $g_{\bm\phi}(\bm{z})$} (20.25,9.75);
            ...
            \node[font=\LARGE] at (23,12.25) {$\bm{\hat{x}}$};
...
\caption{
    Autoencoder architecture. The encoder network $f_{\bm\theta}(\bm{x})$ 
    compresses the input $\bm{x}$ into a latent representation $\bm{z}$, while the decoder 
    network $g_{\bm\phi}(\bm{z})$ reconstructs the input from the latent representation, 
